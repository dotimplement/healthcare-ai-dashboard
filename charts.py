...
@app.cell
def _(load_repos):
    # Data loading and all derived columns live in healthcare_ai/data.py so the
    # dashboard and these charts cannot disagree. The owner taxonomy is in
    # healthcare_ai/owner_types.yaml.
    df_health = load_repos()
    as_of = df_health.attrs["as_of"]
    print(f"Data as of {as_of} (from {as_of.source})")
    return as_of, df_health


@app.cell
def _(chart_year_range, df_health):
    # Derived from the data, never hardcoded: a refresh that adds a new year
    # extends the charts instead of silently dropping it.
    YEAR_RANGE = chart_year_range(df_health)
    print(f"Chart year range: {YEAR_RANGE[0]}-{YEAR_RANGE[1] - 1}")
    return (YEAR_RANGE,)
...
def _(df_health):
    df_health[
        ...
        & (df_health["Created_years"] == df_health["Created_years"].max())
    ]
    return
...
@app.cell
def _(color_palette, df_health, filter_to_chart_years, go):
    def plot_repos_by_owner_type_periods(repos_df):
        ...
        repos_df = filter_to_chart_years(repos_df)
...
@app.cell(hide_code=True)
def _(filter_to_chart_years, pd, star_history_cutoff, timedelta):
    import plotly.graph_objects as go

    # Star history fetching lives in healthcare_ai/stars.py and is run by
    # refresh.py -- it is a pipeline step, not a chart. Regenerate with:
    #     uv run python -m healthcare_ai.stars
    ...
    def plot_star_history(events_df):
        ...
        for owner_type in owner_types:
            ...
            owner_events = owner_events[owner_events["date"] < star_history_cutoff(events_df)]
    ...
    def plot_star_history_bar(events_df):
        ...
        events_df = filter_to_chart_years(events_df)
...
@app.cell
def _(load_star_events):
    events_df = load_star_events()
    return (events_df,)
